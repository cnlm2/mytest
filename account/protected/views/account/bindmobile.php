...
$("#getkey").click(function(){
	...
	var mobile = $("#Account_verifiedmobile").val();
	$.get("/account/index.php/account/sendverifyphone/"+mobile,function(jstr) {
		var data  = eval('('+ jstr +')');
		var res = data['res'];
		if (res==0) {
			timeout = 120;
...
&nbsp;
	...
	<tr class="row">
		...
		<td class="finput"><div>
		<?php echo $form->textField($model,'verifiedmobile',array('size'=>50,'maxlength'=>50)); ?>
		</div></td>
		<td class="fnote">
			<?php echo $form->error($model,'verifiedmobile'); ?>
			请填写安全手机，用于密码找回。一旦填写并验证后将不可以修改。
		</td>
	</tr>
